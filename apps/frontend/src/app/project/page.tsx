...
const TIER3 = [
  ...
  { field: 'kpi', label: 'KPI / метрики', textarea: true },
  { field: 'existingContent', label: 'Существующий контент', textarea: true },
] as const
...
function FieldView({ label, value, onEdit }: { label: string; value?: string | string[]; onEdit: () => void }) {
  ...
  return (
    ...
    >
      ...
      <p className={cn('mt-0.5 text-sm', display ? 'text-foreground' : 'text-muted-foreground/60 italic')}>
...
function ProfilePageInner() {
  ...
  const save = async () => {
    ...
    try {
      const { data } = await apiFetch<{ data: Profile }>(`/api/projects/${activeProject.id}/profile`, {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-[22px] font-medium text-foreground">Профиль проекта</h1>
        {success && (
